...
docker run -i -t --rm --name manaus --volume ${WORKDIR}/manaus/data:/manaus/data --volume ${WORKDIR}/manaus/log:/manaus/log elegansio/manaus:latest /manaus/bin/calculate-keywords-for-sentences-simpler-format --raw_conversations /manaus/data/__data_tokenized.txt --word_frequencies /manaus/data/word_frequency.tsv --output_file /manaus/data/__data_keywords.txt
